...
{ config, lib, pkgs, ... }:
with lib; let
  cfg = config.services.animdb;

  animdbConfig = pkgs.writeText "config.toml" ''
    listen_addr = "${cfg.host}:${toString cfg.port}"
    data_dir = "/var/lib/animdb"
...
in
{
  options.services.animdb = {
    enable = mkEnableOption "Enable the animdb service";
  ...
  config = mkIf cfg.enable {
    systemd.services.animdb = {
      ...
      serviceConfig = {
        ...
        StateDirectory = "animdb";

        ExecStart = "${cfg.package}/bin/animdb serve -c '${animdbConfig}'";
    ...
    users.users = mkIf (cfg.user == "animdb") {
      animdb = {
        group = cfg.group;
        isSystemUser = true;
      };
    ...
    users.groups = mkIf (cfg.group == "animdb") {
      animdb = {};
    };
  };
}
